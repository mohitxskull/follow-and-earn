...
module.exports = async (interaction, client = interaction.client) => {
  ...
  const targetEmbed = new MessageEmbed(interaction.message.embeds[0])
    .setDescription(
      `Thank you for cooperating, your reward points are added to your account for verifying this **[twitter account](https://twitter.com/intent/user?user_id=${task.user.twitter})** follow.`
    )
    .setColor('GREEN');

  await grantPoints(
    client.user,
    [interaction.user, followerUser],
    100,
    client.pool
  );
  await addFollower(
  ...
  const followerEmbed = new MessageEmbed()
    .setDescription(
      `🎊 Congratulations! You recived reward points in your account for following this **[twitter account](https://twitter.com/intent/user?user_id=${task.user.twitter})**`
    )
    .setColor('GREEN');
